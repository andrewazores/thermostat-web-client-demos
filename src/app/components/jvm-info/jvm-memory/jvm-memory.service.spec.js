...
describe('JvmMemoryService', () => {
  ...
  describe('getJvmMemory(jvmId)', () => {
    it('should resolve mock data', done => {
      ...
      httpBackend.when('GET', 'http://example.com:1234/jvm-memory/0.0.2?q=vmId%3D%3Dfoo-jvmId')
        .respond(expected);
      svc.getJvmMemory('foo-jvmId').then(res => {
        res.data.should.deepEqual(expected);
        done();
      });
      httpBackend.expectGET('http://example.com:1234/jvm-memory/0.0.2?q=vmId%3D%3Dfoo-jvmId');
      httpBackend.flush();
      scope.$apply();
    });
  });
